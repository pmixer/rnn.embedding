...
def train():
    ...
    args = TrainArgs('mooc', 'jodie', 50, 128, True, 0.8, 'data/mooc.csv')
    ...
    crossEntropyLoss = nn.CrossEntropyLoss(weight=torch.Tensor([1,true_labels_ratio]))
    MSELoss = nn.MSELoss()
        ...
        for j in range(train_end_idx):
            ...
            if timestamp - tbatch_start_time > tbatch_timespan:
                ...
                for i in range(len(current_tbatches_user)):
                    ...
                    tbatch_userids = torch.LongTensor(current_tbatches_user[i]) # Recall "current_tbatches_user[i]" has unique elements
                    tbatch_itemids = torch.LongTensor(current_tbatches_item[i]) # Recall "current_tbatches_item[i]" has unique elements
                    tbatch_interactionids = torch.LongTensor(current_tbatches_interactionids[i]) 
                    feature_tensor = Variable(torch.Tensor(current_tbatches_feature[i])) # Recall "current_tbatches_feature[i]" is list of list, so "feature_tensor" is a 2-d tensor
                    user_timediffs_tensor = Variable(torch.Tensor(current_tbatches_user_timediffs[i])).unsqueeze(1)
                    item_timediffs_tensor = Variable(torch.Tensor(current_tbatches_item_timediffs[i])).unsqueeze(1)
                    tbatch_itemids_previous = torch.LongTensor(current_tbatches_previous_item[i])
